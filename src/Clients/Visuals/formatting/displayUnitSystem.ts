...
module powerbi {
    ...
    export class DisplayUnitSystem {
        ...
        public format(value: number, format: string, decimals?: number): string {

            if (!DisplayUnitSystem.UNSUPPORTED_FORMATS.test(format)) {
                if (this.displayUnit) {
                    var projectedValue = this.displayUnit.project(value);
                    return this.formatHelper(value, projectedValue, this.displayUnit.labelFormat, format, decimals);
                }
                if (decimals !== undefined) {
                    return this.formatHelper(value, value, '{0}', format, decimals);
                }
            }
        ...
        private formatHelper(value: number, projectedValue: number, labelFormat: string, format: string, decimals?: number) {
                var precision = decimals !== undefined ? Double.pow10(decimals) : Double.getPrecision(value);
            var x = Double.roundToPrecision(projectedValue, precision);

                if (format && !formattingService.isStandardNumberFormat(format))
                return formattingService.formatNumberWithCustomOverride(x, format, labelFormat);

                var text = formattingService.formatValue(x, "G");
            return formattingService.format(labelFormat, [text]);
        }
    ...
    export class DefaultDisplayUnitSystem extends DisplayUnitSystem { 
        ...
        // Methods
        public format(data: number, format: string, decimals?: number): string {
            ...
            return super.format(data, format, decimals);
        }
