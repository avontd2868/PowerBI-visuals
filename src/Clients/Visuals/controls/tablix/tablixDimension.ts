...
module powerbi.visuals.controls {
    ...
    export class TablixRowDimension extends TablixDimension {
        ...
        /**
        * This method first populates the footer folowed by each row and their corelating body cells from top to bottom.
        **/
        ...
        **/
        private addNode(item: any, items: any, rowIndex: number, depth: number): ITablixCell {
            ...
            var cell: ITablixCell = this._tablixLayoutManager.getOrCreateRowHeader(item, items, rowIndex, this._hierarchyNavigator.getLevel(item));
            ...
            if (this._hierarchyNavigator.isLeaf(item)) {
                cell.colSpan = depth - this._hierarchyNavigator.getLevel(item);
                cell.rowSpan = 1;
                if (!match)
                    this.bindRowHeader(item, cell);
                this._tablixLayoutManager.onRowHeaderRealized(item, cell);
                ...
                cell.colSpan = 1;
                cell.rowSpan = this._layoutManager.getRealizedItemsCount() - previousCount + 1;
                if (!match)
                    this.bindRowHeader(item, cell);
                this._tablixLayoutManager.onRowHeaderRealized(item, cell);
            }            
        ...
        private addBodyCells(item: any, items: any, rowIndex: number) {
            var firtsVisibleColumnIndex: number = this._otherDimension.getIntegerScrollOffset();
            ...
            for (var i = 0; i < columnCount; i++) {
                ...
                var cellItem: any = hierarchyNavigator.getIntersection(item, hierarchyNavigator.getLeafAt(otherModel, firtsVisibleColumnIndex + i));
        ...
        private addFooterBodyCells(rowItem: any) {
            ...
            for (var i = 0; i < columnCount; i++) {
                //get column header "item" by index to pair up with row header to find corelating body cell
                var columnItem: any = this._hierarchyNavigator.getLeafAt(this._otherDimension.model, firtsVisibleColumnIndex + i);
