...
module powerbi.visuals {
    ...
    export class ClusteredColumnChartStrategy implements IColumnChartStrategy {
        ...
        public drawColumns(useAnimation: boolean): D3.Selection {
            ...
            if (dataLabelSettings != null) {
                dataLabelLayout = dataLabelUtils.getColumnChartLabelLayout(data, this.getLabelLayoutXY(axisOptions), true, false);
            }
            ...
            if (!this.animator || !useAnimation || result.failed) {
                shapes = ColumnUtil.drawDefaultShapes(data, series, clusteredColumnLayout, ClusteredColumnChartStrategy.classes.item);
        ...
        private getLabelLayoutXY(axisOptions: ColumnAxisOptions): any {
            ...
            return {
                ...
                y: (d: ColumnChartDataPoint) => scaledY0 + AxisHelper.diffScaled(yScale, Math.max(0, d.value), 0) - dataLabelUtils.defaultColumnLabelMargin,
            };
    }
    }
    ...
    export class ClusteredBarChartStrategy implements IColumnChartStrategy {
        ...
        public drawColumns(useAnimation: boolean): D3.Selection {
            ...
            var dataLabelLayout = null;
            if (dataLabelSettings != null) {
                dataLabelLayout = dataLabelUtils.getColumnChartLabelLayout(data, this.getLabelLayoutXY(axisOptions), false,false);
            }
            ...
            if (!this.animator || !useAnimation || result.failed) {
                shapes = ColumnUtil.drawDefaultShapes(data, series, clusteredBarLayout, ClusteredBarChartStrategy.classes.item);
                if (dataLabelLayout !== null) {
                    if (dataLabelSettings.show) {
                        ColumnUtil.drawDefaultLabels(series, dataLabelLayout, { height: this.height, width: this.width });
                }
        ...
        private getLabelLayoutXY(axisOptions: ColumnAxisOptions): any{
            ...
            return {
                x: (d: ColumnChartDataPoint) => scaledX0 + AxisHelper.diffScaled(xScale, Math.max(0, d.value), 0) + dataLabelUtils.defaultColumnLabelMargin,
