    ...
    describe('WaterfallChart', function () {
        ...
        describe('data converter', function () {
            ...
            it('has correct positions', function () {
                expect(data.dataPoints.map(function (d) { return d.position; })).toEqual(positions);
                expect(data.positionMin).toBe(posMin);
                expect(data.positionMax).toBe(posMax);
            });
            it('has correct values', function () {
                expect(data.dataPoints.map(function (d) { return d.value; })).toEqual(values.concat(total));
                expect(data.positionMin).toBe(posMin);
                expect(data.positionMax).toBe(posMax);
            });
            ...
            it('should have no highlights', function () {
                expect(data.dataPoints.some(function (d) { return d.highlight; })).toBe(false);
                expect(data.hasHighlights).toBe(false);
            });
            it('should have no selected data points', function () {
                expect(data.dataPoints.some(function (d) { return d.selected; })).toBe(false);
            });
