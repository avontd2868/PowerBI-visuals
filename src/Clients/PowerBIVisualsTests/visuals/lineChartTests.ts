...
module powerbitests {
    ...
    describe("LineChart Dataview Validation", () => {
        ...
        var seriesColumnRef = powerbi.data.SQExprBuilder.fieldDef({ schema: 's', entity: 'e', column: 'col2' });

        var dataViewMetadata2: powerbi.DataViewMetadata = {
            columns: [
                { name: 'col1' },
                { name: 'col2' }
            ]
        ...
        it('Check convert categorical + fill color',() => {
            ...
            var dataView: powerbi.DataView = {
                ...
                categorical: {
                    categories: [{
                        ...
                        objects: [{ dataPoint: { fill: { solid: { color: seriesColor } } } }]
                    }],
                    values: DataViewTransform.createValueColumns([{
                        source: dataViewMetadata.columns[1],
                        values: [100, 200, 700],
            ...
            var actualData = LineChart.converter(dataView, blankCategoryValue, colors, false);
            var expectedData: powerbi.visuals.LineChartData = {
                series: [{
                    key: key,
                    lineIndex: 0,
                    color: seriesColor,
            ...
            var dataView: powerbi.DataView = {
                ...
                categorical: {
                    categories: [{
                        source: dataViewMetadata.columns[0],
                        values: ['John Domo', 'Delta Force', 'Jean Tablau'],
                        objects: [{ dataPoint: { fill: { solid: { color: seriesColors[0] } } } },
                            { dataPoint: { fill: { solid: { color: seriesColors[1] } } } },
                            { dataPoint: { fill: { solid: { color: seriesColors[2] } } } }]
                    }],
                    values: DataViewTransform.createValueColumns([
                        {
                            source: dataViewMetadata.columns[1],
        ...
        it('Check convert categorical multi-series',() => {
            ...
            var seriesColors = [
                colors.getColor(0).value,
                colors.getColor(1).value,
                colors.getColor(2).value,
            ];
        ...
        it('Check convert non-category multi-measure + fill colors',() => {
                    ...
                    values: DataViewTransform.createValueColumns([
                        {
                            ...
                            values: [200],
                            objects: [{ dataPoint: { fill: { solid: { color: seriesColors[1] } } } }]
                        }])
                }
            };
            ...
            var actualData = LineChart.converter(dataView, blankCategoryValue, colors, false);
            var expectSlices: powerbi.visuals.LineChartData = {
                series: [
                    {
                        key: keys[0],
                        lineIndex: 0,
                    ...
                    data: [
                        {
                            ...
                            key: JSON.stringify({ ser: key, catIdx: 0 }),
                            showLabel: true,
                            labelFill: colors.getColor(0).value,
                        },
                        {
        ...
        function lineChartDomValidation(interactiveChart: boolean) {
            ...
            it('line chart non-category multi-measure dom validation', (done) => {
                var metadata: powerbi.DataViewMetadata = {
                    columns: [
                        { name: 'col1', isMeasure: true, type: DataShapeUtility.describeDataType(SemanticType.Number) },
        ...
        describe("xAxis Validations",() => {
            ...
            function setAxis(xType: any) { //TODO: Change it, to only set Axis values
                ...
                for (var i = 0; i < dots.length; i++) {
                    ...
                    var point: Point = { x: strPoint[0], y: strPoint[1] };
                    points.push(point);
                }
