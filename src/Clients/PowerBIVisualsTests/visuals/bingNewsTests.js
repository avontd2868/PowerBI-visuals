...
(function (powerbitests) {
    var BingNews = powerbi.visuals.BingNews;
    describe("BingNews", function () {
        it('BingNews registered capabilities', function () {
            expect(powerbi.visuals.visualPluginFactory.create().getPlugin('bingNews').capabilities).toBe(BingNews.capabilities);
        });
    });
    describe("BingNews tests", function () {
        var v, element;
        var bingSmallTileDelay = 1000;
        beforeEach(function () {
            element = powerbitests.helpers.testDom('200', '300');
            v = powerbi.visuals.visualPluginFactory.create().getPlugin('bingNews').create();
            v.init({
                element: element,
        ...
        it('BingNews_onDataChange', function () {
            v.onDataChanged({
                dataViews: [{
                    metadata: {
                        columns: []
        ...
        //    }, bingSmallTileDelay);
        //});    
        it('BingNews_OnResize_Large', function () {
            resizeTestSetup();
            v.onResizing({
                height: 500,
                width: element.width()
        ...
        it('BingNews_OnResize_SmallThenLarge', function (done) {
            resizeTestSetup();
            v.onResizing({
                height: 50,
                width: element.width()
